...
//       localStorage.setItem("cart", JSON.stringify(res.data.data.cartItems));
//     }
//   };
...
const StoreContextProvider = (props) => {
  ...
  const fetchFoodlist = async () => {
    ...
    if (cachedFoods) {
      setfood_list(JSON.parse(cachedFoods));
    ...
    try {
      const res = await axios.get(url + "/api/food/list");
      setfood_list(res.data.data);
      localStorage.setItem("foods", JSON.stringify(res.data.data));
    } finally {
      setLoadingFoods(false);
    }
  };
